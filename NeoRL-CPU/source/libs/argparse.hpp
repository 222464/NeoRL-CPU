 ...
 *  \endcode
 *  @link https://github.com/hbristow/argparse
 */
class ArgumentParser {
private:
    class Any;
...
public:
    ...
    const String& retrieve(const String& name, const String& defval) {
        if (index_.count(delimit(name)) == 0) return defval;
        ...
        if (arg.fixed) {
            if (var.castTo<String>().empty()) return defval;
        } else {
            if (var.castTo<StringVector>().size() == 0) return defval;
        }
        
        return variables_[N].castTo<const String>();
    }
